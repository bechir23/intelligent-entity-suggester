...
const extractEntitiesHomogeneityFixed = async (text: string): Promise<EntityMatch[]> => {
  ...
  // Check each cached product and its variations
  Object.keys(productCache).forEach(productKey => {
    ...
    // Create regex for the product key itself
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      if (!isOverlapping(startIndex, endIndex)) {
        ...
        entities.push({
          ...
          confidence: 0.95,
          isFilter: true,
          hoverText: `Product: ${productKey} (database match)`
        });
        console.log(`✅ Product info: ${productKey} (database match from "${match[0]}")`);
      }
    }
    
    // Check variations
    productData.forEach(variation => {
      if (typeof variation === 'string' && variation.length >= 3) {
        ...
        while ((varMatch = varRegex.exec(text)) !== null) {
          ...
          const endIndex = varMatch.index + varMatch[0].length;
          ...
          if (!isOverlapping(startIndex, endIndex)) {
            ...
            entities.push({
              ...
              confidence: 0.85,
              isFilter: true,
              hoverText: `Product: ${productKey} (variation match)`
            });
            console.log(`✅ Product info: ${productKey} (variation "${variation}" from "${varMatch[0]}")`);
          }
        }
      }
    });
