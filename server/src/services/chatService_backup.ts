...
export const chatService = {
  ...
  processQuery: async (message: string, userName?: string): Promise<QueryResult> => {
    try {
      ...
      // Generate representative SQL query for frontend display
      let sqlQuery = '';
      if (suggestedTables.length > 0) {
        const primaryTable = suggestedTables[0];
        sqlQuery = `SELECT * FROM ${primaryTable}`;
        
        // Add WHERE conditions based on entities
        const conditions: string[] = [];
        entities.forEach(entity => {
          if (entity.type === 'info' && entity.value) {
            conditions.push(`name ILIKE '%${entity.value}%'`);
          } else if (entity.type === 'status_filter' && entity.value) {
            conditions.push(`status = '${entity.value}'`);
          } else if (entity.type === 'temporal' && entity.value) {
            conditions.push(`created_at >= '${entity.value}'`);
          } else if (entity.type === 'user_filter' && entity.value) {
            conditions.push(`assigned_to = (SELECT id FROM users WHERE full_name = '${entity.value}')`);
          }
        });
